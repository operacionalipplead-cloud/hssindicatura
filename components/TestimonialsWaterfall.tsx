...
const TESTIMONIALS = [
  ...
  },
  {
    ...
    text: "Pela primeira vez temos assembleias produtivas e sem gritaria. A postura técnica do Danilo traz uma segurança jurídica que nunca tivemos.",
    rating: 5
  },
  {
    id: 3,
...
const TestimonialCard: React.FC<{ data: typeof TESTIMONIALS[0] }> = ({ data }) => (
  <div className="bg-white p-8 rounded-xl shadow-[0_4px_20px_-4px_rgba(3,25,38,0.08)] border border-[#E2E8F0] hover:border-[#C9943C]/50 transition-colors duration-300 flex flex-col gap-4">
...
const TestimonialsWaterfall: React.FC = () => {
  ...
  const col1 = [...TESTIMONIALS.slice(0, 2), ...TESTIMONIALS.slice(0, 2)]; // Duplicate for seamless loop
      ...
      <div className="relative max-w-7xl mx-auto h-[600px] overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
          ...
          <div className="flex flex-col gap-6 animate-scroll-up">
            {col1.map((item, idx) => <TestimonialCard key={`c1-${idx}`} data={item} />)}
            {col1.map((item, idx) => <TestimonialCard key={`c1-dup-${idx}`} data={item} />)}
          </div>

          {/* Column 2 - Down (Slower) */}
          <div className="hidden md:flex flex-col gap-6 animate-scroll-down">
            {col2.map((item, idx) => <TestimonialCard key={`c2-${idx}`} data={item} />)}
            {col2.map((item, idx) => <TestimonialCard key={`c2-dup-${idx}`} data={item} />)}
          </div>

          {/* Column 3 - Up */}
          <div className="hidden md:flex flex-col gap-6 animate-scroll-up-delayed">
            {col3.map((item, idx) => <TestimonialCard key={`c3-${idx}`} data={item} />)}
            {col3.map((item, idx) => <TestimonialCard key={`c3-dup-${idx}`} data={item} />)}
          </div>
        </div>
      </div>
